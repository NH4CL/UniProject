...
namespace MVC.Controls.Grid
{
    public static class GridDataExtensions
    {
        ...
        public static GridData ToGridData<T>(this IQueryable<T> datasource, SearchModel searchModel) where T : class
        {
            IQueryable<T> query = searchModel.ApplyFilters(datasource);
            return query.ToGridData(searchModel.Page, searchModel.Rows);
        }

        public static GridData ToGridData<T>(this IQueryable<T> datasource, SearchModel searchModel, IEnumerable<string> columns) where T:class
        {
            IQueryable<T> query = searchModel.ApplyFilters(datasource);
            return query.ToGridData(searchModel.Page, searchModel.Rows, columns);
        }


        public static GridData ToGridData<T>(this IQueryable<T> datasource, SearchModel searchModel, GridColumnModelList<T> columns) where T:class
        {
            IQueryable<T> query = searchModel.ApplyFilters(datasource);

            if (columns == null)
                return query.ToGridData(searchModel.Page, searchModel.Rows, null);
            else
                return query.ToGridData(searchModel.Page, searchModel.Rows, null).UseValuesFromColumnExpressions(columns);
        }
        ...
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly", MessageId = "Jq",
            Justification = "Grid is the correct name of the grid component we use.")]
        internal static GridData ToGridData<T>(this IQueryable<T> baseList,
            int currentPage,
            int rowsPerPage,
            string orderBy,
            string searchQuery,
