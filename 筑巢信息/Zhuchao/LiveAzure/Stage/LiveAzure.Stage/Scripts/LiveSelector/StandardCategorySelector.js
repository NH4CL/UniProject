function standardCategorySelectTreeObject() {
    this.createTree = function (treeId, categoryType, targetTextbox, targetHidden, onClick) {
        //树状结构的对象
        var zTree1;
        //树属性的设定
        var setting;
        var strAsyncUrl = "/Resource/StandardCategorySelectLoad";
        //var strAsyncParam = asyncParam;													//异步参数不需要，直接指定为对象的Guid        
        var strLoadUrl = "/Resource/StandardCategorySelectLoad";
        ...
        //初始化显示
        if (hidden.val() != "") {
            $.post("/Resource/GetStandardCategoryName", { cid: hidden.val() },
            function (data) {
        ...
        setting = {
            ...
            asyncParamOther: { categoryType: categoryType },
            callback: {
                click: zTreeOnClick
            }
        };
        ...
        function zTreeOnClick(event, treeId, treeNode) {
            if (treeNode.id != "00000000-0000-0000-0000-000000000000" && !treeNode.isParent) {
                ...
                if (onClick != undefined && onClick != "") {
                    eval(onClick + "(treeNode.id, treeNode.name);");
                }
            }
        }
        //绑定TEXTBOX点击弹出地区选择框
